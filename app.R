...
# 2021-12-10: Revised content of the "Help" and "About" modal windows
...
server <- function(input, output, session) {
  ...
  observeEvent(input$about, {
    showModal(modalDialog(
      tags$h2("About VIP_Explorer"),
      tags$p("VIP_Explorer is an R Shiny app based on Beaumont et al. (in review) which allows users to examine relationships between complementarity, climate change and land use impacts beyond that possible in the source publication."),
      tags$p(tags$strong("Very Import Places (VIPs)"), "are those regions which currently support a high proportion of evolutionarily distinct terrestrial tetrapod lineages. VIPs are defined using a threshold applied to the distribution of complementarity values; complementarity is a measure of uniqueness or 'irreplaceability' and may be measured using species richness or phylogenetic diversity. Within our manuscript, we defined a VIP region is one with complementarity values greater than a selected percentile (default 75%) estimated from the global distribution of complementarity values."),
      tags$p("Within VIPs, the app allows users to consider the impact of two primary stressors on biodiversity, exposure to climate change and habitat modification. By adjusting thresholds for 'high' impacts due to these two primary stressors, users can see the distribution of", tags$strong("wicked places"), "that is, those areas within VIPs facing the highest levels of threat from climate change and habitat modification. Like VIPs, thresholds for loss of habitat condition and exposure to climate change are calculated using global distributions of these measure, and a default percentile of 75% is used to estimate threshold values for each. However, data for the two stressors differ from complementarity because they change through time and with selected climate change scenario. Therefore, we must also select a scenario and future time to which we apply thresholds. To make valid comparisons across scenarios and times, thresholds for the two stressors are calculated on the global distribution of values under climate change scenario RCP8.5 and future time 2050, and then applied to the selected scenario and time."),
      ...
      tags$ul(tags$li("Sensitivity of results to the thresholds selected to define VIPs and wicked places"),
              tags$li("Differences in VIP and wicked place results across the major vertebrate clades.")),
      ...
      tags$blockquote(style="font-size:100%;", "Beaumont LJ, Nipperess DA, Wilson PD, Baumgartner JB, Esperon-Rodriguez M (in review) How exposure to land use impacts and climate change may prune the tetrapod tree of life."),
      tags$blockquote(style="font-size:100%;", "Wilson PD, Nipperess DA, Baumgartner JB, Beaumont LJ, Esperon-Rodriguez M (2021) VIP_Explorer: A ShinyApp."),
      ...
      tags$p(style="font-size:85%;", "Land use change data was computed using the Land Use Harmonisation (v 2f; HUH2) dataset (Hurtt et al (2020) Harmonization of global land use change and management for the period 850-2100 (LUH2) for CMIP6. Geoscience Model Development 13:5425-5464."),
      tags$p(style="font-size:85%;", "Species distribution polygons used to compute the spatial distribution of species richness and phylogenetic complementarity were derived from data obtained under licence from:", tags$a("IUCN Red List of Threatened Species v6.2", href = "https://www.iucnredlist.org/resources/spatial-data-download"), ",", tags$a("BirdLife International and the NatureServe (2018) ", href = "http://datazone.birdlife.org/species/requestdis"), "and the", tags$a("Global Assessment of Reptile Distributions (GARD) v1.1", href = "http://www.gardinitiative.org/data.html"), ", Roll et al (2017) The global distribution of tetrapods reveals a need for targeted reptile conservation. Nature Ecology and Evolution 1:1677-1682."),
      tags$p(style="font-size:85%;", "Species taxonomy and nomenclature were standardised to the sources for species distribution. Mismatches were resolved using ", tags$a("GBIF", href = "https://www.gbif.org"), "accessed via the web interface and through the R package", tags$a("rgbif", href = "https://CRAN.R-project.org/package=rgbif"), ", BirdLife International, GARD, and other sources as needed (details provided as supplementary information in Beaumont et al., in review)."),
      tags$p(style="font-size:85%;", "Phylogenetic trees were assembled from Jetz et al (2012) The global diversity of birds in space and time. Nature 491: 444-448; Colston et al (2020) Phylogenetic and spatial distribution of evolutionary diversification, isolation, and threat in turtles and crocodilians (non-avian archosauromorphs). BMC Evolutionary Biology 20: 81; Jetz and Pyron (2018) The interplay of past diversification and evolutionary isolation with present imperilment across the amphibian tree of life. Nature Ecology and Evolution 2: 850-858; Upham et al (2019) Inferring the mammal tree: species-level sets of phylogenies for questions in ecology, evolution, and conservation. PLoS Biology 17: e3000494; Tonini et al (2016) Fully-sampled phylogenies of squamates reveal evolutionary patterns in threat status. Biological Conservation 204: 23-31; Crawford et al (2015) A phylogenomic analysis of turtles. Molecular Phylogenetics and Evolution 83: 250-257."),
  ...
  observeEvent(input$help, {
    showModal(modalDialog(easyClose = TRUE, 
                          ...
                          tags$p("This is a short summary of the concepts and methods used to compute", tags$strong("Very Important Places (VIPs)"), "and", tags$strong("Wicked Places"), "(based on Beaumont et al. (in review) to help you interpret the map generated by this app. Please see the original publication for details."),
                          ...
                          tags$h4(style="color:MediumBlue;", "Complementarity"),
                          tags$p("Choose a combination of", tags$strong("Complementarity type"), "(either Species Richness or Phylogenetic Diversity) and", tags$strong("Clade"), "(e.g. Tetrapoda, Mammalia, Amphibia, Squamata, Aves, Crocodylia, Testudines) from the pull-down selections. VIPs and Wicked places will be re-computed using the current threshold settings when a change is made."),
                          ...
                          tags$p("A layer control is present in the top right corner of the map display. You can choose a background or base map style. You can also choose which of the 4 computed layers are displayed. By default, only", tags$strong("VIP"), "and", tags$strong("Wicked places"), "layers are displayed."),
